...
calculate_max_height <- function(cnum, pnum, dtFiltered) {
  ...
  for (c in cnum) { # for all conditions
    for (p in pnum) { # for all participatns
      # Get all vectors
      yright <- right(p, c)$pos_y
      yleft <- left(p, c)$pos_y
      time <- (right(p, c)$time - right(p, c)$time[1])
...
###  Linear Mixed-Effects
Since there are unequal sample sizes, the ANOVA method cannot be used. Additionally, the data are not normally distributed, and thus using a linear-mixed model regression is invalid (to be improved later).
```{r, echo=FALSE}
...
# Plotting Q-Q plot of the residuals of the fitted model
renderPlot({
  m_h = get_lmer_h()
  qqnorm(resid(m_h))
})
