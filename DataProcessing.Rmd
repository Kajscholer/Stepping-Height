...
observeEvent(input$save, {
  p = ggplot(max_height_data, aes(x = condition, y = max_height, fill = trialNo)) +
    geom_boxplot(aes(group = interaction(condition, trialNo)), outlier.shape = NA) +
    geom_jitter(position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.7)) +
    scale_fill_grey(name = "Trial No", start = 0.2, end = 0.8)  # scale_color_discrete for colors
  ggsave("distances_bp.svg", plot = p, device = "svg") #install.packages("svglite")
...
```{r, echo=FALSE}
get_summ <- reactive(function(column_name){
  return ( ddply(max_height_data, column_name, summarise, grp.mean=mean(distance)) )
})

get_mu <- reactive({
  max_height_data <- calculate_max_height(input$cnum, input$pnum, dtFiltered)
  column_names = colnames(max_height_data)
  mu <- lapply(column_names, get_summ())
  return (setNames(mu, column_names))
})
```
...
dtResiduals_h = reactive({
  max_height_data <- calculate_max_height(input$cnum, input$pnum, dtFiltered)
  mu_h = get_mu()
  max_height_data$residual = 999 # just fill with some placeholder value
  max_height_data$mean = 999

  for (i in 1:length(mu_h$condition$grp.mean)){
    row_h = mu_h$condition[i,]
    max_height_data[condition==row_h$condition]$mean = row_h$grp.mean
    max_height_data[condition==row_h$condition]$residual = 
      max_height_data[condition==row_h$condition]$max_height - row_h$grp.mean
  }
  return (max_height_data)
})
...
renderPrint({
  max_height_data <- calculate_max_height(input$cnum, input$pnum, dtFiltered)
  max_height_data = dtResiduals_h()

  print(leveneTest(max_height ~ condition, max_height_data))
  print(bartlett.test(max_height ~ condition, max_height_data))
})

renderPlot({
  max_height_data <- calculate_max_height(input$cnum, input$pnum, dtFiltered)
  max_height_data = dtResiduals_h()
  ggplot(max_height_data,
         aes(residual)) +
           geom_density(fill = "gray")
})
...
```
###  Linear Mixed-Effects
Since there are unequal sample sizes, the ANOVA method canot be used. Additionally, the data are not normally distrubuted, and thus using a linear-mixed model regression is invalid (to be improved later).
```{r, echo=FALSE}
...
get_lmer_h = reactive({
  ...
  m_h <- lmer(max_height ~ condition + (1 | participant), data=max_height_data)
  return (m_h)
})
...
renderPrint({
  ...
  print("..........")
  print(anova(m_h))
  print(eta_sq(m_h, partial=TRUE))
  #print(summary(partial_eta_squared(m)))
  print(r.squaredGLMM(m_h))
  m_h.emm.s = emmeans(m_h, list(pairwise ~ condition), adjust = "bonferroni")
  print(m_h.emm.s)
  m_h.eff = eff_size(m_h.emm.s, sigma = sigma(m_h), edf = 561)
  print(m_h.eff)
})
...
makeBoxPlots_h = reactive({
  ...
  p_h = ggplot(dtResiduals_h(), aes(x = condition, y = max_height)) +
    geom_boxplot() + geom_jitter() + stat_pvalue_manual(stat.test,label= "p.adj.signif")#, y.position = 35, step.increase = 0.1,hide.ns = TRUE)
  return (p_h)
})
...
# Filter the DT based on selected inputs
observeEvent(input$save, {
  ggsave("boxplot.svg", plot = makeBoxPlots_h(), device = "svg")
})
```